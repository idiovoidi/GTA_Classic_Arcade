...
class ZoneManager {
    ...
    isLocationSuitable(x, y, size) {
        ...
        // Check for road access (simplified)
        const roadPos = this.game.city.getRandomRoadPosition();
        const distanceToRoad = Math.sqrt(
            Math.pow(x - roadPos.x, 2) + Math.pow(y - roadPos.y, 2)
        );
        
        return distanceToRoad < 200; // Must be within 200 units of a road
    }
